...
describe('Accounts Payable Concurrency', () => {
      ...
      $transaction: jest.fn().mockImplementation(async (cb: any) => {
        return lock.acquire(async () => {
          ...
          const txMock = {
            ...
            supplierInvoice: {
              findFirst: prismaMock.supplierInvoice?.findFirst,
              update: prismaMock.supplierInvoice?.update,
            },
            journalEntry: {
              create: jest
                .fn()
  ...
  it('1. 100 parallel invoice number allocations should produce 100 unique sequential numbers without duplicates', async () => {
    const tasks = Array.from({ length: 100 }, () =>
      numberingService.nextNumber(mockOrgId, 'SUPPLIER_INVOICE', mockUserId),
    );
    ...
    expect(formattedNumbers[0]).toBe('SI-000001');
    expect(formattedNumbers[99]).toBe('SI-000100');
  });

  it('2. 100 parallel attempts to post the same invoice should result in exactly 1 success and 99 failures', async () => {
    ...
    const baseInvoice = {
      ...
      supplier: { id: 'supp-1', name: 'Acme Supplies' },
      currency: { id: 'c-1', code: 'USD', symbol: '$' },
      lines: [],
    };

    prismaMock.supplierInvoice.findFirst = jest.fn().mockImplementation(() => {
      return Promise.resolve({
        ...baseInvoice,
    ...
    prismaMock.supplierInvoice.update = jest.fn().mockImplementation(() => {
      if (hasBeenPosted) {
    ...
    const attempts = Array.from({ length: 100 }, () =>
      invoiceService
        .post(mockOrgId, mockInvoiceId, mockUserId)
